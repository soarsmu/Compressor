...
from model import Roberta, distill_loss, patience_loss
...
def teacher_predict(model, args, loader):
    ...
    for batch in bar:
        ...
        with torch.no_grad():
            ...
            logits.append(logit)
            hiddens.append([x.detach().to("cpu") for x in hidden])
...
def student_train(T_model, S_model, args, train_loader, test_loader):
    try:
        ...
        t_train_hiddens = torch.load("./train_hiddens.bin")
        # t_test_hiddens = torch.load("./test_hiddens.bin")
    except:
        t_train_logits, t_train_hiddens = teacher_predict(T_model, args, train_loader)
        # t_test_logits, t_test_hiddens = teacher_predict(T_model, args, test_loader)
        torch.save(t_train_logits, "./train_logits.bin")
        # torch.save(t_test_logits, "./test_logits.bin")
        torch.save(t_train_hiddens, "./train_hiddens.bin")
        # torch.save(t_test_hiddens, "./test_hiddens.bin")
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            s_logits, s_hiddens = S_model(texts)

            loss = distill_loss(s_logits, t_train_logits[step], label)
            pat_loss = 0
            for i, j in zip([0, 1], [10, 11]):
                pat_loss += patience_loss(s_hiddens[i], t_train_hiddens[step][j].to("cuda"), normalized_patience=True)
            loss += 0.05 * pat_loss
            loss.backward()
