...
from models.clone_detection.BigCloneBench.model import Model
...
def get_example(item):
    url1,url2,label,tokenizer, args=item
    
    try:
        code=' '.join(url1.split())
    except:
        code=""
    code1=tokenizer.tokenize(code)
    try:
        code=' '.join(url2.split())
    except:
        code=""
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        try:
            ...
            data = []
            with open("../dataset/adv_test.jsonl") as fi:
                for line in fi:
                    line = line.strip()
                    lin=json.loads(line)
                    code1 = lin["func"]
                    code2 = lin["ori"]
                    la = int(lin["target"])
                    data.append((code1, code2, la, tokenizer, args))
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for param_tensor in model.state_dict():
            if param_tensor.split(".")[-1] == "weight" and not param_tensor.split(".")[-2] == "LayerNorm" \
                and not param_tensor.split(".")[-3] == "classifier" and not param_tensor.split(".")[-3] == "embeddings":
                # np.save("tmp/"+param_tensor+".npy", model.state_dict()[param_tensor].numpy())
                gobo_weight = np.load("bins_8_OT_-4/tmp/"+param_tensor + ".npy")
                model.state_dict()[param_tensor].copy_(torch.tensor(gobo_weight))
        # exit()
        model.to(args.device)
