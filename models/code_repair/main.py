...
def train(args, model, tokenizer):
    ...
    train_dataset = TensorDataset(all_source_ids,all_source_mask,all_target_ids,all_target_mask)
    ...
    nb_tr_examples, nb_tr_steps,tr_loss,global_step,best_bleu = 0, 0,0,0,0
    ...
    for idx in range(0, int(args.num_train_epochs)):
        ...
        for batch in bar:
            ...
            tr_loss += loss.item()
            tr_num += 1
            bar.set_description("epoch {} loss {}".format(idx, round(tr_loss/tr_num,5)))
            ...
            if args.save_steps > 0 and global_step % args.save_steps == 0:
                ...
                logger.info("  "+"*"*20)
                logger.info("  Current BLEU:%s", round(results["eval_bleu"], 2))
                logger.info("  Current ACC:%s", round(results["eval_acc"], 2))
                logger.info("  Best BLEU:%s", round(best_bleu, 2))
                logger.info("  "+"*"*20)
                
                if results["eval_bleu"]>best_bleu:
                    best_bleu=results["eval_bleu"]
